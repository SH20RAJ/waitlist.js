'use client';

import React, { useState } from 'react';
import { motion } from 'framer-motion';

type WidgetPreviewProps = {
  waitlist: {
    id: string;
    name: string;
    description: string;
    widgetSettings: string;
    brandColor: string;
  };
};

export default function WidgetPreview({ waitlist }: WidgetPreviewProps) {
  const [email, setEmail] = useState('');
  const [name, setName] = useState('');
  const [submitted, setSubmitted] = useState(false);
  
  // Parse widget settings
  const settings = waitlist.widgetSettings ? JSON.parse(waitlist.widgetSettings) : {
    theme: 'light',
    layout: 'standard',
    fields: [
      { name: 'email', label: 'Email', type: 'email', required: true },
      { name: 'name', label: 'Name', type: 'text', required: false }
    ],
    submitButton: {
      text: 'Join Waitlist',
      color: waitlist.brandColor || '#0ea5e9'
    },
    successMessage: 'Thanks for joining our waitlist!',
    referralEnabled: true,
    referralMessage: 'Share with friends to move up the list!'
  };
  
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
  };
  
  const handleReset = () => {
    setEmail('');
    setName('');
    setSubmitted(false);
  };
  
  return (
    <div className="border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
      {!submitted ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <h3 className="text-xl font-bold mb-2">{waitlist.name}</h3>
          {waitlist.description && (
            <p className="text-gray-600 mb-6">{waitlist.description}</p>
          )}
          
          <form onSubmit={handleSubmit}>
            {settings.fields.map((field: any) => (
              <div key={field.name} className="mb-4">
                <label htmlFor={field.name} className="block text-sm font-medium text-gray-700 mb-1">
                  {field.label} {field.required && <span className="text-red-500">*</span>}
                </label>
                <input
                  type={field.type}
                  id={field.name}
                  value={field.name === 'email' ? email : name}
                  onChange={(e) => field.name === 'email' ? setEmail(e.target.value) : setName(e.target.value)}
                  required={field.required}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                  placeholder={`Enter your ${field.label.toLowerCase()}`}
                />
              </div>
            ))}
            
            <button
              type="submit"
              style={{ backgroundColor: settings.submitButton.color }}
              className="w-full px-4 py-2 text-white rounded-md hover:opacity-90 transition-opacity"
            >
              {settings.submitButton.text}
            </button>
          </form>
        </motion.div>
      ) : (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-center py-8"
        >
          <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
            </svg>
          </div>
          
          <h3 className="text-xl font-bold mb-2">{settings.successMessage}</h3>
          
          {settings.referralEnabled && (
            <div className="mt-6">
              <p className="text-gray-600 mb-4">{settings.referralMessage}</p>
              <div className="bg-gray-50 p-4 rounded-md mb-4">
                <p className="text-sm font-medium mb-1">Your referral link:</p>
                <div className="flex">
                  <input
                    type="text"
                    readOnly
                    value={`${window.location.origin}/w/${waitlist.id}?ref=ABC123`}
                    className="flex-1 px-3 py-2 border border-gray-300 rounded-l-md focus:ring-primary-500 focus:border-primary-500 text-sm"
                  />
                  <button
                    className="inline-flex items-center px-3 py-2 border border-l-0 border-gray-300 rounded-r-md text-sm font-medium text-gray-700 bg-gray-50 hover:bg-gray-100"
                  >
                    Copy
                  </button>
                </div>
              </div>
              
              <div className="flex space-x-4 justify-center">
                <button className="p-2 bg-blue-100 text-blue-600 rounded-full">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </button>
                <button className="p-2 bg-blue-100 text-blue-600 rounded-full">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                  </svg>
                </button>
                <button className="p-2 bg-green-100 text-green-600 rounded-full">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                </button>
              </div>
            </div>
          )}
          
          <button
            onClick={handleReset}
            className="mt-6 text-primary-600 hover:text-primary-700 text-sm font-medium"
          >
            Reset Preview
          </button>
        </motion.div>
      )}
    </div>
  );
}
